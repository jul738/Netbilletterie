...
//=============================================
	...
	$result_article = mysql_query( $rqSql_article )or die( "Ex�cution requ�te impossible_article.");
	$row = mysql_fetch_array( $result_article); 
	$article = $row["article"]; 
	$article= addslashes($article);
	$date = $row["date"]; 
	
	//on recup�re les info du client1 (caisse du soir) pour la 1er ligne de la page
	...
	$rqSql4= "SELECT id_tarif, nom_tarif, prix_tarif, DATE_FORMAT(saison, '%d/%m/%Y' ) AS date FROM ".$tblpref."tarif
		...
		ORDER BY nom_tarif ASC";
		 $result4 = mysql_query( $rqSql4 ) or die( "Ex�cution requ�te impossible.");
?>
...
if (!res){ 
alert("Vous n'avez pas renseign� le Tarif"); 
return res; // Je sors de la fonction avec le r�sultat "false" 
} 
} 
</script>
			...
			<center>
				<form name='formu' method='post' action='bon_suite_soir.php' onSubmit="return verif_formulaire()">
					<table class="boiteaction">
						...
						<tr>
							<td class="texte0">Choisir la quantit&eacute; d'entr�es vendu pour ce tarif </td>
