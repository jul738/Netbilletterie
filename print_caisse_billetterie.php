...
$annee_2= $annee_1 -1;
//=============================================
//on recup�re les infos de la caisse billetterie par num d'enregistrement
...
//on recup�re le total en caisse
...
<h1>Caisse billetterie</h1>
	<h2>La caisse des especes est actuellement de <FONT face="Comic Sans MS" color="red"><?php echo "$total_caisse $devise"; ?></FONT></h2>
			Avec <br/>
			<?php
			//On montre ce qu'il y a dans la caissse � ce moment 
				if($pos!="0"){
					$sql1 = "SELECT
					...
					while($data = mysql_fetch_array($req1))
						...
						$nombre = $data['nbr1'];
						$espece = $data['espece'];
						echo "En esp�ce de $espece $devise = $total $devise <br> ";
						}
				}
				if($neg!="0"){
